...
function generateOutput(spec, paths) {
  return new Promise((resolve) => {
    const verifiedPaths = paths.filter(path => path !== null);
    const relativeSpecPath = path.relative(process.cwd(), spec)
    if (!verifiedPaths.length) {
      logger.info(`\n> goplanet found ${chalk.green(0)} leaky tests affecting ${chalk.underline(relativeSpecPath)}`);
    } else {
      logger.info(`\n> goplanet found ${chalk.red(paths.length)} leaky ${verifiedPaths.length > 1 ? 'tests' : 'test'} affecting ${chalk.underline(relativeSpecPath)}:\n`);
...
function execute(options, spec, dir, done) {
  getAllTestPaths(dir, options.patterns)
    ...
    .then((paths) => {
      return generateOutput(spec, paths)
    })
    .then((paths) => {
